...
export function WeatherWidget() {
  ...
  useEffect(() => {
    // Open-Meteo API for Ollantaytambo (-13.2589, -72.2635)
    fetch("https://api.open-meteo.com/v1/forecast?latitude=-13.2589&longitude=-72.2635&current=temperature_2m,precipitation,uv_index")
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <div style={{
      ...
      <h3 style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--color-deep)", display: "flex", alignItems: "center", gap: "0.5rem", margin: 0 }}>
        🌤️ {locale === "zh" ? "实时天气与高原提示" : locale === "es" ? "Clima y Consejos de Altitud" : "Live Weather & Altitude Tips"}
      </h3>
      ...
      <p style={{ margin: 0, fontSize: "0.95rem", color: "var(--color-earth)", lineHeight: 1.5 }}>
        {locale === "zh" && `当前紫外线指数：${uvLevel} (${weather.uv})。请备好防晒霜！海拔 2792 米，比库斯科更适合作为适应高反的首日住宿地。`}
        {locale === "en" && `Current UV Index: ${uvLevel} (${weather.uv}). Apply sunscreen! Altitude is 2,792m, an ideal first-day stop to acclimatize better than Cusco.`}
        {locale === "es" && `Índice UV actual: ${uvLevel} (${weather.uv}). ¡Use protector solar! La altitud es de 2,792m, ideal para aclimatarse el primer día en lugar de Cusco.`}
        {locale === "qu" && `Inti k'anchay: ${uvLevel} (${weather.uv}). 2792m, Cusco llaqtamanta aswan allin soroche amachakunapaq.`}
      </p>
    </div>
  );
}
